...
const TaskRow = ({
  ...
  // editing helpers
  types = [],
  typeOptions = [], // [{name, color, id}] from Notion database
  courseOptions = [], // [{id, name}]
}) => {
    ...
    <div className="notion-row">
      ...
      {editCourse ? (
        <select
          ...
          onChange={(e) => { /* commit on change for quicker UX */ const val = e.target.value || null; commit({ courseId: val }); }}
        >
      ...
      {editWorth ? (
        <input
          ...
          onBlur={(e) => {
            ...
            const finalValue = Math.round(clamped); // enforce integer
